...
def audio_tower(shape=(13, 20, 1), drop_ratio=0.5, kreg=1e-4, weights=None):
    ...
    x = Flatten()(x)
    x = Dense(256, activation='relu', name='s_av_a_fc7')(x)  # fc7
...
def video_tower(shape=(5, 100, 100, 3), drop_ratio=0.5, kreg=1e-4, weights=None):
    ...
    x = Flatten()(x)
    x = Dense(256, activation='relu', name='s_av_v_fc7')(x)  # fc7

    return inputs, x


def static_av(audio_shape=(13, 20, 1), video_shape=(5, 100, 100, 3), drop_ratio=0.5, num_classes=2, weights=None):
    # Define audio tower
    ...
    x = Dense(512, activation='relu', name='s_av_fc1')(x)
    x = Dropout(drop_ratio)(x)
